...
- **Language:** Python. Local development uses **3.13.3** (the git-ignored `venv/`); CI pins **3.13** (`.github/workflows/test.yml:21`). `pyproject.toml` declares `requires-python = ">=3.11"`, so 3.11 is the enforced floor — code must remain compatible across 3.11–3.13.
...
- Package declared via `pyproject.toml` (setuptools src-layout manifest; declares `requires-python = ">=3.11"` but no runtime deps — those stay in `requirements.txt`). No `setup.py`, `setup.cfg`, `Pipfile`, or `poetry.lock`.
...
- Committing a Ruff config and adding a CI lint/format gate is planned but **not yet done** (`plans/018-ci-lint-format-gate.md`, TODO); a follow-on pre-commit hook is plan 021 (TODO, blocked on 018).
...
- `.github/workflows/test.yml:13,21,23-30` (ubuntu-latest, Python 3.13, `pip install -e .` + pytest)
...
- `pyproject.toml` (setuptools src-layout manifest; `requires-python = ">=3.11"`; no `[tool.ruff]` or runtime deps declared)
